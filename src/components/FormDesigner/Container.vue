<template>
  ...
  <el-container class="widget-config-container">
    <el-header style="height:auto;padding-left:0px">
      ...
      <div :class="{active: configTab=='formcontainer'}"
           class="config-tab2"
           @click="handleConfigSelect('formcontainer')">表单设计</div>
      <div :class="{active: configTab=='listcontainer'}"
           class="config-tab2"
           @click="handleConfigSelect('listcontainer')">列表设计</div>
      <span v-if="selectform!==''"
      ...
      <el-dialog :visible.sync="dialogFormVisible"
        ...
        <el-select allow-create
                   ...
                   placeholder="请选择"
                   filterable
                   style="display:inline-block;width:85%!important;margin-right:5px;">
      ...
      <el-aside style="width:150px">
        ...
        <div class="components-list">
          ...
          <draggable :list="basicComponents"
            ...
            <li v-for="(item, index) in basicComponents"
                :key="index"
          ...
          <draggable :list="advanceComponents"
                     :options="{group:{ name:'people', pull:'clone',put:false},sort:false, ghostClass: 'ghost'}"
                     :move="handleMove"
                     element="ul"
                     @end="handleMoveEnd"
          ...
          <draggable :list="layoutComponents"
                     :options="{group:{ name:'people', pull:'clone',put:false},sort:false, ghostClass: 'ghost'}"
                     :move="handleMove"
                     element="ul"
                     @end="handleMoveEnd"
      ...
      <el-aside class="widget-config-container"
        ...
        <el-container>
          <el-header height="45px">
            <div :class="{active: configTab=='widget'||configTab=='formcontainer'}"
                 class="config-tab"
                 @click="handleConfigSelect('widget')">字段属性</div>
            <div :class="{active: configTab=='form'}"
                 class="config-tab"
                 @click="handleConfigSelect('form')">表单属性</div>

          </el-header>
          <el-main class="config-content">
            <widget-config v-show="configTab=='widget'||configTab=='formcontainer'"
    ...
    <el-container v-show="configTab=='listcontainer'">
      ...
      <list-config :config="widgetForm.config"
