<template>
  <div id="movie" class="app-container">
    <h2>音乐查询</h2>
    <el-input v-model="criteria" placeholder="请输入歌名" style="padding-bottom:10px;"> @keyup.enter.native="search">
    ...
    <el-table v-loading.body="listLoading" empty-text="暂无数据" :data="tableData" element-loading-text="拼命加载中" fit highlight-current-row>
      ...
      <el-table-column label="歌手" align="center">
        <template slot-scope="scope">
          <el-tag v-for="(item,index) in scope.row.attrs.singer" :key="index" type="primary" style="margin-right:5px">{{ item }}</el-tag>
      ...
      <el-table-column label="描述" align="center">
        <template slot-scope="scope">
          <el-tag v-if="scope.row.alt_title.length>0" type="danger" style="margin-right:5px;margin-top:10px">{{ scope.row.alt_title }}</el-tag>
