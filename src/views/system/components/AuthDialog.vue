...
<template>
  ...
  <el-dialog v-if="dialog.visible"
    ...
    <GenerateForm ref="generateDialogForm"
                  ...
                  :remote="remoteFuncs"
                  :setReadOnly="{
                    whiteList:['realname','username','password','rank','deptid']
                  }"/>
    <el-form size="small"
    ...
    showDialog(param = {}, status = STATUS.CREATE, formValues = {}) {
      ...
      getFormDetail('auth').then((res) => {
        ...
        if (this.dialog.status === STATUS.UPDATE) {
          ...
          this.$nextTick(() => {
            // 设置树上的选中项
            this.$refs.deptTree.setCheckedKeys(formValues.userdeptpermission.split(','));
            // this.$refs.unitTree.setCheckedKeys(formValues.unituserpermission.split(','));
          });
        } else {
          this.formValues = {};
        }
    ...
    btnSave_onClick() {
      ...
      this.$refs.generateDialogForm
        ...
        .then((formValue) => {
          ...
          opt.userdeptpermission = this.$refs.deptTree.getCheckedKeys(true).join(',');
          // opt.unituserpermission = this.$refs.unitTree.getCheckedKeys().join(',');

          crud(type, 'users', opt).then(() => {
